...
def Correlation(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        ...
        if pred_std * actual_std != 0: correlation = cov / (pred_std * actual_std)
        else: correlation = 0
        return correlation
...
class SimpleMLP(nn.Module):
    ...
    def __init__(self,
        ...
        self.main = nn.Sequential(
            ...
            nn.Sigmoid())
            #nn.Softmax(dim=1)) #nn.Softmax(dim=1) nn.Sigmoid()
...
class MLMHead(nn.Module):
    ...
    def forward(self, hidden_states, feature=None, targets=None):
        ...
        if targets is not None: 
            ...
            masked_lm_loss = loss_fct(logits, targets)
            acc_fct = Accuracy(ignore_index=self._ignore_index)
            metrics = {'Correlation': Correlation(logits, targets)}
...
class ValuePredictionHead(nn.Module):
    def __init__(self,
        ...
        self.classifier = nn.Sequential(
            weight_norm(nn.Linear(hidden_size, 128), dim=None),
            nn.ReLU(),
            nn.Dropout(0.2, inplace=True),
            weight_norm(nn.Linear(128, self.task_size), dim=None)) #self.task_size

    def forward(self, hidden_states, feature=None, targets=None):
        if feature != None:
            hidden_states = torch.cat((hidden_states, feature), 2)
        hidden_states = self.classifier(hidden_states)
        hidden_states = torch.squeeze(hidden_states, -1)
        outputs = (hidden_states,)
        if targets is not None:
            ...
            positive_weights = (targets > 0) * 1
            negative_weights = (targets == 0) * 0.01
            weights = positive_weights + negative_weights
            masked_lm_loss = torch.mean(loss_fct(logits, targets) * weights)

            valid_mask = (targets > 0)
            metrics = {'Correlation': Correlation(targets[valid_mask], logits[valid_mask])}
            loss_and_metrics = (masked_lm_loss, metrics)
            outputs = (loss_and_metrics,) + outputs
...
class MultiValuePredictionHead(nn.Module):
    def __init__(self,
        ...
        self.task_size = task_size
        self._ignore_index = ignore_index
        self.classifier = SimpleMLP(hidden_size*2, 512, self.task_size, 0.1)

    def forward(self, hidden_states, feature_output=None, high_ids=None, low_ids=None):
        if feature_output != None:
            hidden_states = torch.cat((hidden_states, feature_output), 1)
        hidden_states = self.classifier(hidden_states)
        outputs = (hidden_states,)
        ...
        if high_ids is not None or low_ids is not None:
            ...
            valid_mask = (targets >= 0)
            loss_fct = nn.MSELoss(reduce=False) #reduce=False
            valid_targets = targets[valid_mask]
            valid_logits = hidden_states[valid_mask]

            #masked_lm_loss = loss_fct(valid_logits, valid_targets)
            
            positive_weights = (valid_targets == 1) * 1
            negative_weights = (valid_targets == 0) * 3
            weights = positive_weights + negative_weights
...
class SequenceValuePredictionHead(nn.Module):
    def __init__(self,
                 hidden_size: int,
                 task_size: int,
                 hidden_act: typing.Union[str, typing.Callable] = 'gelu',
        ...
        self.task_size = task_size
        self._ignore_index = ignore_index
        self.classifier = SimpleMLP(hidden_size, hidden_size, self.task_size, 0.1)
